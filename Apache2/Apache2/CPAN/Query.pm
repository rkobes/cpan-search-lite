...
sub meta : method {
  ...
  my $path_info = $r->path_info;
  my $mode = 'dist';
  my ($page, $dist_name, $results, $type);
  if ($path_info =~ m!^/([^/]+)/META\.(.*)!) {
    $dist_name = $1;
    $type = $2;
    $type = undef unless $types{$type};
  }
  else {
    $page = 'letters';
  }
  if ($dist_name and $type) {
    if ($dist_name =~ /::/) {
      $dist_name =~ s/::$//;
      $query->query(mode => 'module',
		    name => $dist_name, user_fields => 'dist_id');
      if (my $mod_results = $query->{results}) {
	$query->query(mode => $mode, id => $mod_results->{dist_id});
	$results = $query->{results};
	$page = $results ? $type : 'letters';
      }
      else {
	$page = 'letters';
      }
  ...
  $r->content_type("text/x-$type; charset=UTF-8") if ($page ne 'letters');
  my %extra_info;
  unless (ref($results) eq 'ARRAY') {
    if (my $name = $results->{$mode_info->{$mode}->{name}}) {
      $extra_info{title} = sprintf("%s : %s",
				   $self->{title},
				   $name);
      $extra_info{meta} = 1 unless ($page eq 'letters');
    }
  }
  if (my $error = $query->{error}) {
    $r->log->error($error);
