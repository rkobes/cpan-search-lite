...
sub insert {
  ...
  my @fields = qw(cpanid email fullname homepage);
  ...
  foreach my $cpanid (keys %$data) {
    ...
    $sth->execute($cpanid, $values->{email}, 
		  $values->{fullname}, $values->{homepage})
      or do {
        $cdbi->db_error($sth);
        $self->{error_msg} = $cdbi->{error_msg};
        return;
...
This value, if true, indicates that an F<Makefile.PL> file for
the distribution is available.

=item * build

This value, if true, indicates that an F<Build.PL> file for
the distribution is available.
