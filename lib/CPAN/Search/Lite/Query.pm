...
sub info {
    ...
    $args{fields} = $args{user_fields} || 
      [ qw(auth_id cpanid email fullname homepage) ];
    $args{table} = 'auths';
...
sub info {
    ...
    $args{fields} = $args{user_fields} ||
      [ qw(dist_id dist_name dist_abs dist_vers md5
           dist_file size birth readme changes meta install makefile build
           auth_id cpanid fullname) ];
    $args{table} = 'dists';
...
#    }
    ...
    $args{table} = 'mods';
    $args{join} = {reqs => 'mod_id', dists => 'dist_id'};
    $args{fields} = [ qw(mod_id req_vers mod_name mod_abs dist_name) ];
    $args{order_by} = 'mod_name';
...
C<install>, C<auth_id>, C<cpanid>, C<makefile>, C<build> and C<fullname>
of the C<auths>, C<mods>, and C<dists> tables. Note that
C<readme>, C<changes>, C<meta>, C<makefile>, C<build>,
and C<install> are boolean values
