...
sub whois {
  my $self = shift;
  my $whois = catfile $self->{CPAN}, 'authors', '00whois.xml';
  print "Reading information from $whois\n";
  XML::SAX->add_parser(q(XML::SAX::ExpatXS));
  my $factory = XML::SAX::ParserFactory->new();
  my $handler = WhoisHandler->new();
  my $parser = $factory->parser( Handler => $handler);

  eval { $parser->parse_uri($whois); };
  if ($@) {
    print "Error in parsing $whois: $@\n";
    return;
  }
  $self->{auths} = $handler->{auths};
}
...
sub new {
  my $type = shift;
  return bless {text => '', auths => {}, cpanid => {}}, $type;
}
...
sub start_element {
  ...
  if ($curr_el eq 'cpanid') {
    if (my $cpanid = $self->{cpanid}) {
      for (qw(id email homepage fullname)) {
	$cpanid->{$_}->{value} = undef;
      }
    }
  }
  $self->display_text();
...
sub end_element {
  ...
  if ($curr_el eq 'cpanid') {
    ...
    if (defined  $cpanid->{id}->{value}) {
      $self->{auths}->{$cpanid->{id}->{value}} = 
	{fullname => $cpanid->{fullname}->{value} || '',
         email => $cpanid->{email}->{value} || 
         (lc $cpanid->{id}->{value} . '@cpan.org'),
	 homepage => $cpanid->{homepage}->{value} || '',
	};
    }
#   print STDERR $cpanid->{id}->{value}, "****", $email, $/;
  }
...
sub display_text {
  ...
  if ( defined( $self->{text} ) && $self->{text} ne "" ) {
    $cpanid->{$curr_el}->{value} = $self->{text};
