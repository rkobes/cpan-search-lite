...
our %wanted = map {$_ => 1} qw(REPOSITORYSUMMARY SOFTPKG ABSTRACT ARCHITECTURE);
our $arch = '';
...
sub summary {
  ...
  my @attempts = ($url =~ /trouchelle.com|ppm4/) ? 
    ('package.xml', @tries) : @tries;
  ...
  my $packages = parse($url, $file, $type);
  unlink $file;
...
sub parse {
  ...
  if ($type =~ m{application/.*zip} or $url =~ m{://(trouchelle.com|ppm4)} ) {
    unless ($file =~ /\.gz$/) {
      rename $file, "$file.gz";
      return unless (gunzip("$file.gz"));
    }
  }
...
sub gunzip {
  ...
  if (! open(my $fh, '>', $outfile) ) {
	...
	$success = 0;
	print "Error reading from $infile: " . ($gzerrno+0) . "\n" ;
      }
      else {
	$success = 1;
      }
...
sub start_element {
  ...
  foreach my $ak (keys %{ $element->{Attributes} } ) {
    ...
    my $name = $at->{Name};
    my $value = $at->{Value};
    if ($curr_el eq 'REPOSITORYSUMMARY' and $name eq 'ARCHITECTURE') {
       $rep_arch = $value;
       last;
    }
    $ppd->{keep} = 1 
      if ( ($curr_el eq 'ARCHITECTURE' and $value eq $arch) 
          or (defined $rep_arch and $rep_arch eq $arch) );
